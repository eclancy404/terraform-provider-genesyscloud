...
func buildEvaluationAssignments(evaluations []interface{}, pp *policyProxy, ctx context.Context) (error, *[]platformclientv2.Evaluationassignment) {
	...
	for _, assignEvaluation := range evaluations {
		...
		if evaluationFormId != "" {
			form, _, err := pp.getFormsEvaluation(ctx, evaluationFormId)
			if err != nil {
				return err, nil
			} else {
	...
	return nil, &assignEvaluations
}

func flattenEvaluationAssignments(assignments *[]platformclientv2.Evaluationassignment, pp *policyProxy, ctx context.Context) (error, []interface{}) {
	if assignments == nil {
		return nil, nil
	}
	...
	for _, assignment := range *assignments {
		...
		if assignment.EvaluationForm != nil {
			...
			if err != nil {
				return fmt.Errorf("failed to get evaluation form versions %s %v", *assignment.EvaluationForm.Name, resp), nil
			} else {
	...
	return nil, evaluationAssignments
}
...
func buildMeteredEvaluationsTimeInterval(interval []interface{}) *platformclientv2.Timeinterval {
	...
	if len(interval) <= 0 || (len(interval) == 1 && interval[0] == nil) {
		return nil
	}
...
func buildAssignMeteredEvaluations(assignments []interface{}, pp *policyProxy, ctx context.Context) (error, *[]platformclientv2.Meteredevaluationassignment) {
	...
	for _, assignment := range assignments {
		...
		if evaluationFormId != "" {
			...
			if err != nil {
				return fmt.Errorf("failed to read media evaluation form %s: %s %v", evaluationFormId, err, resp), nil
			} else {
				evaluationFormContextId := form.ContextId
	...
	return nil, &meteredAssignments
}

func flattenAssignMeteredEvaluations(assignments *[]platformclientv2.Meteredevaluationassignment, pp *policyProxy, ctx context.Context) (error, []interface{}) {
	if assignments == nil {
		return nil, nil
	}
...
func buildAssignMeteredAssignmentByAgent(assignments []interface{}, pp *policyProxy, ctx context.Context) (error, *[]platformclientv2.Meteredassignmentbyagent) {
	...
	for _, assignment := range assignments {
		...
		if evaluationFormId != "" {
			form, resp, err := pp.getFormsEvaluation(ctx, evaluationFormId)
			if err != nil {
				return fmt.Errorf("failed to read evaluation form %s: %s %v", evaluationFormId, err, resp), nil
			} else {
				evaluationFormContextId := form.ContextId
				temp.EvaluationForm = &platformclientv2.Evaluationform{Id: &evaluationFormId, ContextId: evaluationFormContextId}
			}
...
func flattenAssignMeteredAssignmentByAgent(assignments *[]platformclientv2.Meteredassignmentbyagent, pp *policyProxy, ctx context.Context) (error, []interface{}) {
	if assignments == nil {
		return nil, nil
	}
...
func buildAssignCalibrations(assignments []interface{}, pp *policyProxy, ctx context.Context) (error, *[]platformclientv2.Calibrationassignment) {
	...
	return nil, &calibrationAssignments
}

func flattenAssignCalibrations(assignments *[]platformclientv2.Calibrationassignment, pp *policyProxy, ctx context.Context) (error, []interface{}) {
	if assignments == nil {
		return nil, nil
	}
...
func buildAssignSurveys(assignments []interface{}, pp *policyProxy, ctx context.Context) (error, *[]platformclientv2.Surveyassignment) {
	...
	for _, assignment := range assignments {
		...
		if surveyFormName != "" {
			form, _, err := pp.getQualityFormsSurveyByName(ctx, surveyFormName)
			if err != nil {
				return fmt.Errorf("error requesting survey forms %s: %v", surveyFormName, err), nil
			} else {
...
func buildInitiateScreenRecording(initiateScreenRecording []interface{}) *platformclientv2.Initiatescreenrecording {
	if len(initiateScreenRecording) <= 0 {
		return nil
	}
...
func buildPolicyActionsFromMediaPolicy(actions []interface{}, pp *policyProxy, ctx context.Context) (error, *platformclientv2.Policyactions) {
	if len(actions) <= 0 {
		return nil, nil
	}
	...
	err, assignMeteredAssignmentByAgent := buildAssignMeteredAssignmentByAgent(actionsMap["assign_metered_assignment_by_agent"].([]interface{}), pp, ctx)
	if err != nil {
		return fmt.Errorf("unable to build assignMeteredAssignmentByAgent in the buildPolicyActionsFromMediaPolicy() method: %s", err), nil
	}

	err, assignMeteredEvaluations := buildAssignMeteredEvaluations(actionsMap["assign_metered_evaluations"].([]interface{}), pp, ctx)
	if err != nil {
		return fmt.Errorf("unable to build assignMeteredEvaluations in the buildPolicyActionsFromMediaPolicy() method: %s", err), nil
	}

	err, evaluationAssignments := buildEvaluationAssignments(actionsMap["assign_evaluations"].([]interface{}), pp, ctx)
	if err != nil {
		return fmt.Errorf("unable to build evaluation assignments in the buildPolicyActionsFromMediaPolicy() method: %s", err), nil
	}

	err, calibrations := buildAssignCalibrations(actionsMap["assign_calibrations"].([]interface{}), pp, ctx)
	if err != nil {
		return fmt.Errorf("unable to build calibrations in the buildAssignCalibrations() method: %s", err), nil
	}

	err, surveys := buildAssignSurveys(actionsMap["assign_surveys"].([]interface{}), pp, ctx)
	if err != nil {
		return fmt.Errorf("unable to build surveys in the buildAssignSurveys() method: %s", err), nil
	}
...
func flattenPolicyActions(actions *platformclientv2.Policyactions, pp *policyProxy, ctx context.Context) (error, []interface{}) {
	...
	if actions.AssignEvaluations != nil {
		err, actionsMap["assign_evaluations"] = flattenEvaluationAssignments(actions.AssignEvaluations, pp, ctx)
	}

	if actions.AssignMeteredEvaluations != nil {
		err, actionsMap["assign_metered_evaluations"] = flattenAssignMeteredEvaluations(actions.AssignMeteredEvaluations, pp, ctx)
	}

	if actions.AssignMeteredAssignmentByAgent != nil {
		err, actionsMap["assign_metered_assignment_by_agent"] = flattenAssignMeteredAssignmentByAgent(actions.AssignMeteredAssignmentByAgent, pp, ctx)
	}

	if actions.AssignCalibrations != nil {
		err, actionsMap["assign_calibrations"] = flattenAssignCalibrations(actions.AssignCalibrations, pp, ctx)
	}
...
func buildCallMediaPolicyConditions(callMediaPolicyConditions []interface{}) *platformclientv2.Callmediapolicyconditions {
	if len(callMediaPolicyConditions) <= 0 {
		return nil
	}
	...
	teams := teamIdsFromMap(conditionsMap, "team_ids")
...
func flattenCallMediaPolicyConditions(conditions *platformclientv2.Callmediapolicyconditions, schemaTeamIds []string) []interface{} {
	if conditions == nil {
		return nil
	}
	...
	conditionsMap["team_ids"] = flattenTeamIds(conditions.Teams, schemaTeamIds)
...
func buildChatMediaPolicyConditions(chatMediaPolicyConditions []interface{}) *platformclientv2.Chatmediapolicyconditions {
	if len(chatMediaPolicyConditions) <= 0 {
		return nil
	}
...
func flattenChatMediaPolicyConditions(conditions *platformclientv2.Chatmediapolicyconditions, schemaTeamIds []string) []interface{} {
	if conditions == nil {
		return nil
	}
...
func buildEmailMediaPolicyConditions(emailMediaPolicyConditions []interface{}) *platformclientv2.Emailmediapolicyconditions {
	if len(emailMediaPolicyConditions) <= 0 {
		return nil
	}
...
func flattenEmailMediaPolicyConditions(conditions *platformclientv2.Emailmediapolicyconditions, schemaTeamIds []string) []interface{} {
	if conditions == nil {
		return nil
	}
...
func buildMessageMediaPolicyConditions(messageMediaPolicyConditions []interface{}) *platformclientv2.Messagemediapolicyconditions {
	if len(messageMediaPolicyConditions) <= 0 {
		return nil
	}
...
func flattenMessageMediaPolicyConditions(conditions *platformclientv2.Messagemediapolicyconditions, schemaTeamIds []string) []interface{} {
	if conditions == nil {
		return nil
	}
...
func buildCallMediaPolicy(callMediaPolicy []interface{}, pp *policyProxy, ctx context.Context) (error, *platformclientv2.Callmediapolicy) {
	...
	return nil, &platformclientv2.Callmediapolicy{
		Actions:    actions,
		Conditions: buildCallMediaPolicyConditions(policyMap["conditions"].([]interface{})),
	}
}

func flattenCallMediaPolicy(callMediaPolicy *platformclientv2.Callmediapolicy, pp *policyProxy, ctx context.Context, schemaTeamIds []string) (error, []interface{}) {
	if callMediaPolicy == nil {
		return nil, nil
	}

	callMediaPolicyMap := make(map[string]interface{})
	var err error
	err = nil
	if callMediaPolicy.Actions != nil {
		err, callMediaPolicyMap["actions"] = flattenPolicyActions(callMediaPolicy.Actions, pp, ctx)
	}

	flattenConditionsFn := func(conditions *platformclientv2.Callmediapolicyconditions) []interface{} {
		return flattenCallMediaPolicyConditions(conditions, schemaTeamIds)
	}
	resourcedata.SetMapInterfaceArrayWithFuncIfNotNil(callMediaPolicyMap, "conditions", callMediaPolicy.Conditions, flattenConditionsFn)

	return err, []interface{}{callMediaPolicyMap}
}

func buildChatMediaPolicy(chatMediaPolicy []interface{}, pp *policyProxy, ctx context.Context) (error, *platformclientv2.Chatmediapolicy) {
	if len(chatMediaPolicy) <= 0 {
		return nil, nil
	}
	...
	err, actions := buildPolicyActionsFromMediaPolicy(policyMap["actions"].([]interface{}), pp, ctx)
	if err != nil {
		return fmt.Errorf("unable to build poliction actions from media policy in buildChatMediaPolicy: %s", err), nil
	}

	return nil, &platformclientv2.Chatmediapolicy{
		Actions:    actions,
		Conditions: buildChatMediaPolicyConditions(policyMap["conditions"].([]interface{})),
	}
}

func flattenChatMediaPolicy(chatMediaPolicy *platformclientv2.Chatmediapolicy, pp *policyProxy, ctx context.Context, schemaTeamIds []string) (error, []interface{}) {
	...
	if chatMediaPolicy.Actions != nil {
		err, chatMediaPolicyMap["actions"] = flattenPolicyActions(chatMediaPolicy.Actions, pp, ctx)
	}

	flattenConditionsFn := func(conditions *platformclientv2.Chatmediapolicyconditions) []interface{} {
		return flattenChatMediaPolicyConditions(conditions, schemaTeamIds)
	}
	resourcedata.SetMapInterfaceArrayWithFuncIfNotNil(chatMediaPolicyMap, "conditions", chatMediaPolicy.Conditions, flattenConditionsFn)

	return err, []interface{}{chatMediaPolicyMap}
}

func buildEmailMediaPolicy(emailMediaPolicy []interface{}, pp *policyProxy, ctx context.Context) (error, *platformclientv2.Emailmediapolicy) {
	if len(emailMediaPolicy) <= 0 {
		return nil, nil
	}
...
func flattenEmailMediaPolicy(emailMediaPolicy *platformclientv2.Emailmediapolicy, pp *policyProxy, ctx context.Context, schemaTeamIds []string) (error, []interface{}) {
	...
	if emailMediaPolicy.Actions != nil {
		err, emailMediaPolicyMap["actions"] = flattenPolicyActions(emailMediaPolicy.Actions, pp, ctx)
	}

	flattenConditionsFn := func(conditions *platformclientv2.Emailmediapolicyconditions) []interface{} {
		return flattenEmailMediaPolicyConditions(conditions, schemaTeamIds)
	}
	resourcedata.SetMapInterfaceArrayWithFuncIfNotNil(emailMediaPolicyMap, "conditions", emailMediaPolicy.Conditions, flattenConditionsFn)

	return err, []interface{}{emailMediaPolicyMap}
}

func buildMessageMediaPolicy(messageMediaPolicy []interface{}, pp *policyProxy, ctx context.Context) (error, *platformclientv2.Messagemediapolicy) {
	if len(messageMediaPolicy) <= 0 {
		return nil, nil
	}
	...
	err, actions := buildPolicyActionsFromMediaPolicy(policyMap["actions"].([]interface{}), pp, ctx)
	if err != nil {
		return fmt.Errorf("error while invoking buildPoliycyActionsFromMediaPolicy in buildMessageMediaPolicy() function: %s", err), nil
	}

	return nil, &platformclientv2.Messagemediapolicy{
		Actions:    actions,
		Conditions: buildMessageMediaPolicyConditions(policyMap["conditions"].([]interface{})),
	}
}

func flattenMessageMediaPolicy(messageMediaPolicy *platformclientv2.Messagemediapolicy, pp *policyProxy, ctx context.Context, schemaTeamIds []string) (error, []interface{}) {
	...
	if messageMediaPolicy.Actions != nil {
		err, messageMediaPolicyMap["actions"] = flattenPolicyActions(messageMediaPolicy.Actions, pp, ctx)
	}

	flattenConditionsFn := func(conditions *platformclientv2.Messagemediapolicyconditions) []interface{} {
		return flattenMessageMediaPolicyConditions(conditions, schemaTeamIds)
	}
	resourcedata.SetMapInterfaceArrayWithFuncIfNotNil(messageMediaPolicyMap, "conditions", messageMediaPolicy.Conditions, flattenConditionsFn)

	return err, []interface{}{messageMediaPolicyMap}
}

func buildMediaPolicies(d *schema.ResourceData, pp *policyProxy, ctx context.Context) (*platformclientv2.Mediapolicies, error) {
	sdkMediaPolicies := platformclientv2.Mediapolicies{}
	var err error //Placeholder error that will be set in the iff conditions below
	...
	if mediaPolicies, ok := d.Get("media_policies").([]interface{}); ok && len(mediaPolicies) > 0 {
		...
		if callPolicy := mediaPoliciesMap["call_policy"]; callPolicy != nil {
			err, sdkMediaPolicies.CallPolicy = buildCallMediaPolicy(callPolicy.([]interface{}), pp, ctx)
			if err != nil {
				return nil, err
			}
		}
		...
		if chatPolicy := mediaPoliciesMap["chat_policy"]; chatPolicy != nil {
			err, sdkMediaPolicies.ChatPolicy = buildChatMediaPolicy(chatPolicy.([]interface{}), pp, ctx)
			if err != nil {
				return nil, err
			}
		}
		...
		if emailPolicy := mediaPoliciesMap["email_policy"]; emailPolicy != nil {
			err, sdkMediaPolicies.EmailPolicy = buildEmailMediaPolicy(emailPolicy.([]interface{}), pp, ctx)
			if err != nil {
				return nil, err
			}
		}
		...
		if messagePolicy := mediaPoliciesMap["message_policy"]; messagePolicy != nil {
			err, sdkMediaPolicies.MessagePolicy = buildMessageMediaPolicy(messagePolicy.([]interface{}), pp, ctx)
			if err != nil {
				return nil, err
			}
		}
	}
	return &sdkMediaPolicies, err
}

func flattenMediaPolicies(mediaPolicies *platformclientv2.Mediapolicies, pp *policyProxy, ctx context.Context, d *schema.ResourceData) (error, []interface{}) {
	if mediaPolicies == nil {
		return nil, nil
	}

	var schemaMediaPolicies []interface{}
	if mediaPoliciesConfig, ok := d.Get("media_policies").([]interface{}); ok {
		schemaMediaPolicies = mediaPoliciesConfig
	}
	...
	if mediaPolicies.CallPolicy != nil {
		err, mediaPoliciesMap["call_policy"] = flattenCallMediaPolicy(mediaPolicies.CallPolicy, pp, ctx, teamIdsFromMediaPolicyBlock(schemaMediaPolicies, "call_policy"))
	}
	if mediaPolicies.ChatPolicy != nil {
		err, mediaPoliciesMap["chat_policy"] = flattenChatMediaPolicy(mediaPolicies.ChatPolicy, pp, ctx, teamIdsFromMediaPolicyBlock(schemaMediaPolicies, "chat_policy"))
	}
	if mediaPolicies.EmailPolicy != nil {
		err, mediaPoliciesMap["email_policy"] = flattenEmailMediaPolicy(mediaPolicies.EmailPolicy, pp, ctx, teamIdsFromMediaPolicyBlock(schemaMediaPolicies, "email_policy"))
	}
	if mediaPolicies.MessagePolicy != nil {
		err, mediaPoliciesMap["message_policy"] = flattenMessageMediaPolicy(mediaPolicies.MessagePolicy, pp, ctx, teamIdsFromMediaPolicyBlock(schemaMediaPolicies, "message_policy"))
	}

	return err, []interface{}{mediaPoliciesMap}
}
...
func flattenConditions(conditions *platformclientv2.Policyconditions, schemaTeamIds []string) []interface{} {
...
func buildPolicyActionsFromResource(d *schema.ResourceData, pp *policyProxy, ctx context.Context) (*platformclientv2.Policyactions, error) {
	if actions, ok := d.Get("actions").([]interface{}); ok && len(actions) > 0 {
		actionsMap, ok := actions[0].(map[string]interface{})
		if !ok {
			return nil, fmt.Errorf("unable to retrive actions map record in the buildPolicyActionsFromResource method()")
		}
		...
		if err != nil {
			return nil, fmt.Errorf("error with buildAssignMeteredAssignmentByAgent() in buildPolicyActionsFromResource() method %s", err)
		}

		err, assignMeteredEvaluations := buildAssignMeteredEvaluations(actionsMap["assign_metered_evaluations"].([]interface{}), pp, ctx)
		if err != nil {
			return nil, fmt.Errorf("buildAssignMeteredEvaluation() in buildPolicyActionsFromResource() method %s", err)
		}

		err, evaluationAssignments := buildEvaluationAssignments(actionsMap["assign_evaluations"].([]interface{}), pp, ctx)
		if err != nil {
			return nil, fmt.Errorf("error with buildEvaluationAssignments() in buildPolicyActionsFromResource() method %s", err)
		}

		err, calibrations := buildAssignCalibrations(actionsMap["assign_calibrations"].([]interface{}), pp, ctx)
		if err != nil {
			return nil, fmt.Errorf("buildAssignCalibrations() in buildPolicyActionsFromResource() method %s", err)
		}

		err, surveys := buildAssignSurveys(actionsMap["assign_surveys"].([]interface{}), pp, ctx)
		if err != nil {
			return nil, fmt.Errorf("error with buildAssignSurveys() in buildPolicyActionsFromResource(): %s", err)
		}
...
func buildTeamIds(teamIds []interface{}) []platformclientv2.Team {
	teams := make([]platformclientv2.Team, 0)
	for _, id := range lists.InterfaceListToStrings(teamIds) {
		teamId := id
...
func teamIdsFromMap(targetMap map[string]interface{}, key string) []platformclientv2.Team {
	raw, ok := targetMap[key]
	if !ok || raw == nil {
		return buildTeamIds(nil)
	}

	teamIds, ok := raw.([]interface{})
	if !ok {
		return buildTeamIds(nil)
	}

	return buildTeamIds(teamIds)
}

func teamIdsFromConditionBlock(conditions []interface{}) []string {
	if len(conditions) == 0 {
		return nil
	}
	...
	return teamIdsFromMapValue(conditionsMap["team_ids"])
}

func teamIdsFromMediaPolicyBlock(mediaPolicies []interface{}, policyKey string) []string {
	if len(mediaPolicies) == 0 {
		return nil
	}
	...
	policyRaw, ok := mediaPoliciesMap[policyKey]
	if !ok || policyRaw == nil {
		return nil
	}

	policyList, ok := policyRaw.([]interface{})
	...
	conditionsRaw, ok := policyMap["conditions"]
	...
	return teamIdsFromConditionBlock(conditionsList)
}

func teamIdsFromMapValue(raw interface{}) []string {
	if raw == nil {
		return nil
	}
	...
	return lists.InterfaceListToStrings(teamIds)
}

func organizeTeamIdsForRead(schemaList, apiList []string) []string {
...
func teamIdsFromAPI(teams *[]platformclientv2.Team) []string {
...
func flattenTeamIds(teams *[]platformclientv2.Team, schemaTeamIds []string) []string {
	return organizeTeamIdsForRead(schemaTeamIds, teamIdsFromAPI(teams))
}
