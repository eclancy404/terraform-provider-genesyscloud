...
func TestFetchPagesConcurrentlyPreservesPageOrder(t *testing.T) {
	...
	results, resp, err := FetchPagesConcurrently(
		WithMaxConcurrentPages(context.Background(), 2),
		"test_resource",
		[]int{1},
...
func TestFetchPagesConcurrentlyRespectsContextOverride(t *testing.T) {
	resetClientPool()
	defer resetClientPool()

	SdkClientPool = newTestPaginationPool(4)
	...
	results, _, err := FetchPagesConcurrently(
		WithMaxConcurrentPages(context.Background(), 1),
		"test_resource",
		[]int{1},
		&platformclientv2.APIResponse{},
		4,
	...
	assert.Equal(t, []int{2, 3, 4}, fetchedPages)
}
